...
public class CustomWorldGenerator implements IWorldGenerator {
                ...
                if (oreInfo.dimensionId == world.provider.dimensionId) {
                    ...
                    if (oreInfo.biomeId == null || Arrays.asList(oreInfo.biomeId).contains(biome.biomeID))
                        this.addOreSpawn(oreBlock, toSpawnMetadata, toReplace, world, random, chunkX * 16, chunkZ * 16,
                                oreInfo.minVeinSize, oreInfo.maxVeinSize, oreInfo.chancesToSpawn, oreInfo.minY,
                                oreInfo.maxY);
                }
            }
        }
    ...
    public void addOreSpawn(Block block, int metadata, Block toReplace, World world, Random random, int blockXPos,
        ...
        if (metadata == 0) {
            ...
            minable = new WorldGenMinable(block, (minVeinSize + random.nextInt(maxVeinSize - minVeinSize)), metadata,
                    toReplace);
        }
